    ...
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      ...
      <div class="p-6">
        ...
        <div class="flex items-center space-x-4 mb-6">
          ...
          />
          <div>
            <h4 class="text-2xl font-bold text-gray-900">{{ profile.name }}</h4>
            <p class="text-gray-500">{{ profile.username || 'No username' }}</p>
            <span
              class="inline-block px-3 py-1 text-sm font-medium rounded-full mt-2"
              :class="getRelationBadgeClass(profile.relation)"
            >
              {{ formatRelation(profile.relation) }}
            </span>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="space-y-4">
            ...
            <div v-if="profile.date_of_birth" class="flex items-center space-x-3">
            ...
            <div v-if="profile.location" class="flex items-center space-x-3">
              <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div v-if="profile.profession" class="flex items-center space-x-3">
              <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div>
                ...
                <p class="text-gray-900">{{ profile.profession }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div v-if="profile.passion" class="flex items-center space-x-3">
              <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
              </svg>
              <div>
                <p class="text-sm text-gray-500">Passion</p>
                <p class="text-gray-900">{{ profile.passion }}</p>
              </div>
            </div>

            <div v-if="profile.mission" class="flex items-center space-x-3">
              <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/>
              </svg>
              <div>
                <p class="text-sm text-gray-500">Mission</p>
                <p class="text-gray-900">{{ profile.mission }}</p>
              </div>
            </div>

            <div v-if="profile.calling" class="flex items-center space-x-3">
              <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
              </svg>
              <div>
                <p class="text-sm text-gray-500">Calling/Purpose</p>
                <p class="text-gray-900">{{ profile.calling }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-200">
          ...
          <button
            v-if="profile.id && profile.id !== currentUserId"
...
const viewFullProfile = () => {
  if (props.profile.id) {
    window.open(`/profile/${props.profile.id}`, '_blank');
  }
};
</script>
