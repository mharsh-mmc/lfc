...
<template>
    ...
    <div class="min-h-screen bg-[#f7f0e6] font-lato relative">
        ...
        <div class="hidden lg:flex min-h-screen items-stretch">
            <!-- Left: Code Verification Form (2/3) -->
        ...
        <div class="hidden sm:block lg:hidden min-h-screen">
            ...
            <div class="h-1/2 flex flex-col justify-center px-8 lg:px-16 py-10 bg-[#f7f0e6]">
                <div class="mb-8">
                    ...
                    <h2 class="text-3xl font-semibold mb-2 font-playfair text-center">Enter Code</h2>
                ...
                <form @submit.prevent="submit" class="space-y-6 max-w-md mx-auto">
                    ...
                    <div>
                        <label for="code-tablet" class="block text-sm font-medium mb-1">Code</label>
                        <input id="code-tablet" v-model="form.code" type="text" maxlength="6" required placeholder="Enter 6-digit code" class="w-full rounded-md border border-gray-300 px-4 py-3 focus:ring-2 focus:ring-[#1a237e] focus:outline-none bg-white text-base" />
                        <div v-if="form.errors.code" class="mt-1 text-sm text-red-600">{{ form.errors.code }}</div>
                    </div>
                    <button type="submit" :disabled="form.processing" class="w-full bg-[#1a237e] text-white py-3 rounded-md font-semibold text-lg hover:bg-[#283593] transition disabled:opacity-50 mt-2">
                        Verify Code
        ...
        <div class="sm:hidden min-h-screen flex items-center justify-center p-4 relative z-10">
            <div class="w-full max-w-md bg-white/90 backdrop-blur-sm rounded-xl shadow-2xl p-8">
                <div class="mb-8">
                    <div class="text-2xl font-bold mb-6 tracking-tight font-playfair text-center">liveforever</div>
                    <h2 class="text-2xl font-semibold mb-2 font-playfair text-center">Enter Code</h2>
                    <p class="text-gray-600 mb-6 text-center">Enter the 6-digit code sent to your email.</p>
                </div>
                <form @submit.prevent="submit" class="space-y-6">
                    <div>
                    ...
                    <div>
                        <label for="code-mobile" class="block text-sm font-medium mb-1">Code</label>
                        <input id="code-mobile" v-model="form.code" type="text" maxlength="6" required placeholder="Enter 6-digit code" class="w-full rounded-md border border-gray-300 px-4 py-3 focus:ring-2 focus:ring-[#1a237e] focus:outline-none bg-white text-base" />
                        <div v-if="form.errors.code" class="mt-1 text-sm text-red-600">{{ form.errors.code }}</div>
                    </div>
                    <button type="submit" :disabled="form.processing" class="w-full bg-[#1a237e] text-white py-3 rounded-md font-semibold text-lg hover:bg-[#283593] transition disabled:opacity-50 mt-2">
                        Verify Code
